...
Engine::MResult Engine::Main(int move, int curonly)
{
  ...
  //XXX Info.Control(PANEL_PASSIVE, FCTL_GETPANELDIR, MAX_FILENAME, (LONG_PTR)cur_dir);
  //XXX Info.Control(PANEL_PASSIVE, FCTL_GETPANELINFO, 0, (LONG_PTR)&pi);
  // fixed by Nsky: bug #40
//  _toansi(pi.CurDir);

  if(true) //XXX pi.Plugin)
  {
    dstpath="plugin:";
    allowPlug=1;
  }
...
*/
  ...
  //XXX Info.Control(PANEL_ACTIVE, FCTL_GETPANELDIR, MAX_FILENAME, (LONG_PTR)cur_dir);
